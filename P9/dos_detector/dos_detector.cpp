...
//    end.
    ...
    while(i < log.size() && (log[i].time < System().time()) )
    {
        if (!System().is_banned(log[i].ip))
        {
          ...
          //Process this ip according to the algorithm.
            if (counters.find(log[i].ip) == true){ // Si encontramos la IP
                counters.set_value(counters.get_value() + 1); // Sumamos uno al contador, ya que ha vuelto a acceder
            }
            else{
                counters.insert(log[i].ip, 1); // Si no, la insertamos con contador 1
            }
            
            if (counters.get_value() >= max_acc){
                System().ban_ip(counters.get_key(), System().time()); // Si supera el numero maximo de peticiones, la banearemos
            }
    ...
    while(j < log.size()  &&  ((log[j].time + 60) < System().time()))
    {
        if (! (log[j].ip == IP(0,0,0,0)) )
        {
            ...
            if (counters.find(log[j].ip) && counters.get_value() > 0){ // Si encontramos la IP y su contador es mayor que 0, le restamos 1
